    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let isLogin = UserDefaults.standard.value(forKey: "isLogin") as? Bool, let member_name = UserDefaults.standard.value(forKey: "member_name") as? String {
            if(isLogin) {
                ...
                let alertController = UIAlertController(title: "未登入", message:
                    "未登入狀態僅能查看留言！", preferredStyle: UIAlertController.Style.alert)
                alertController.addAction(UIAlertAction(title: "確定", style: UIAlertAction.Style.default,handler: nil))
                self.present(alertController, animated: true, completion: nil)
                inputTextField.isEnabled = false
                inputTextField.backgroundColor = #colorLiteral(red: 0.501960814, green: 0.501960814, blue: 0.501960814, alpha: 1)
                sendMsgBtn.isEnabled = false
                sendPhotoBtn.isEnabled = false
