//
//  MemberHistoryTableViewController.swift
//  DrinkShopClient_IOS
//
...
class MemberHistoryTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if(isLogin){
            ...
            communicator.findOrderHistoryByMemberId(member_id: id){ (result, error) in
                if let error = error {
                ...
                if self.objects.count == 0 {
                    ...
                    let alertController = UIAlertController(title: "無資料", message:
                        "您沒有歷史訂單資料！", preferredStyle: UIAlertController.Style.alert)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.backgroundColor = #colorLiteral(red: 0.852401934, green: 0.852401934, blue: 0.852401934, alpha: 1)
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let selectedIndexPath = tableView.indexPathForSelectedRow {
            ...
            // 取得下一頁
            let memberHistoryVC = segue.destination as!
            MemberHistoryViewController
          
            memberHistoryVC.order = order
        }
    }
